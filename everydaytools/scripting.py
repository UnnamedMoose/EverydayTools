...
def runBatch(cmds, pwd, outStr = -1, errStr = -1, log=True, showPid=False):
	...
	for cmd in cmds:
		...
		with open(logfilePath, "w") as logfile:
			...
			# handle outputs
# TODO this stuff stopped working some time ago, probably because of Python version.
#			if log:
#				outIter = iter(p.stdout.readline, "")
#				for lineOut in outIter:
#					logfile.write(b2s(lineOut))
...
if __name__ == "__main__":
	...
	def allrunTest(param):
		runBatch(["mkdir testCase_{:d}".format(param)], pwd=workDir, log=False, showPid=True)
		return param
